...
public class TableOrdersManager implements OrdersManager {
    ...
    public int itemsQuantity(String itemName) {
        int count = 0;
        for (Orderable order : orders) count += order.itemQuantity(itemName);
        return count;
    }
    ...
    public int[] freeTableNumbers() {
        ...
        for (int i = 0; i < orders.length; i++) {
            if (orders[i] == null) arrayListNumbers.add(i);
        }
        int[] arrNumbers = new int[arrayListNumbers.size()];
        for (int i = 0; i < arrNumbers.length; i++) arrNumbers[i] = arrayListNumbers.get(i);
        return arrNumbers;
    }
